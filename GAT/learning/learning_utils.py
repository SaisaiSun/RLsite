...
from GAT.config.graph_keys import GRAPH_KEYS, TOOL
from GAT.utils import misc
...
def compute_outputs(model, validation_loader):
    ...
    F1 = []
    threshold =0.5
    outpath = '/home/MultiModRLBP/output/'
    ...
    for batch_idx, (graph, indxs, features, seqs_old, seqs, seqlens, len_graphs, chain_idx, rna, chain_id) in enumerate(validation_loader):
        ...
        #id_num+=1
        #f = open(outpath+str(id_num)+'_result.txt','w')
        #f.write(str(true1)+','+str(y_pred))
        #f.close()
        ...
        f1_score = f1_score(true1, y_pred)
    ...
    print("F1_score avg:", sum(F1)/len(F1)) 
    print("--------------------------------")
    ...
    f1_score_all = 2*(precision_all * recall_all) / (recall_all + precision_all)
    print("F1  all:", f1_score_all)
    print(f'TP :{ TPALL}   FP:{FPALL}    TN:{TNALL}     FN:{FNALL}'  )
    mcc =  sum(MCC)/len(MCC)
    precision = sum(Precision)/len(Precision)
    recall = sum(Recall)/len(Recall)
    f1score = sum(F1)/len(F1)
    return true, predicted, mcc, precision,  recall, f1score
...
def evaluate_model_supervised(model, validation_loader, evaluation_function=roc_auc_score):
    ...
    """
    true, predicted, mcc, precision,  recall, f1_score = compute_outputs(model, validation_loader)
    auc_score = evaluation_function(true, predicted)
    auprc = average_precision_score(true, predicted)
    Pre, Rec, _ = precision_recall_curve(true, predicted)
    fpr, tpr, thresholds = roc_curve(true, predicted)
    best_idx = np.argmax(tpr - fpr)
    best_threshold = thresholds[best_idx]
    print(best_threshold)

    return fpr, tpr, auc_score, auprc, mcc, precision,  recall, f1_score, Pre, Rec
