...
class BCEFocalLoss(torch.nn.Module):
    ...
    def forward(self, _input, target):
        ...
        pt = _input
        min_pt = torch.tensor(0.1, dtype=torch.float32)
        pt = torch.max(pt, min_pt)
        alpha = self.alpha
        gamma = self.gamma
        #print(pt)
        #print(target)
        loss = - (1 - pt) ** gamma * target * alpha * torch.log(pt) - \
               pt ** gamma * (1 - target) * (1-alpha) * torch.log(1 - pt)
        #print(loss)
...
def compute_outputs(model, validation_loader):
    ...
    for batch_idx, (graph, indxs, features, seqs_old, seqs, seqlens, len_graphs, chain_idx, rna, chain_id) in enumerate(validation_loader):
        # Get data on the devices
        #graph = send_graph_to_device(graph, device)
        ...
        with torch.no_grad():
            ...
            out = model(graph, features, indxs, seqs_old, seqs, seqlens, len_graphs, chain_idx, rna, chain_id)
            pos = 0
            idx = 0
...
def evaluate_model_supervised(model, validation_loader, evaluation_function=roc_auc_score):
    ...
    auc = evaluation_function(true, predicted)
...
def train_supervised(model,
    ...
    for epoch in range(learning_routine.num_epochs):
        ...
        for batch_idx, (graph, indxs, features, seqs_old, seqs, seqlens, len_graphs, chain_idx, rna, chain_id) in enumerate(train_loader):
            # Get data on the devices
            # graph = send_graph_to_device(graph, device)
           ...
           # mse_loss = torch.nn.MSELoss()(out, newlabel)
            ...
            for idx in range(len(newlabels)):
                #print(out[idx].shape)
                #logicloss +=  BCEFocalLoss()(out[idx],newlabels[idx])
                #logicloss +=  F.binary_cross_entropy(out[idx],newlabels[idx],weight=weights)
                #logicloss +=  torch.nn.BCELoss(weight=weights)(out[idx],newlabels[idx])
                if labels[idx] > 0 :
        ...
        if learning_routine.validation_loader is None:
            ...
            validation_fpr, validation_tpr, validation_auc, validation_mcc, validation_precision, validation_recall, validation_f1score = evaluate_model_supervised(model, validation_loader=learning_routine.validation_loader)
            early_stop = learning_routine.early_stopping_routine(validation_loss=validation_auc, epoch=epoch,
                                                                 model=model, optimizer=optimizer)
            validation_fpr3, validation_tpr3, validation_auc3, validation_mcc3, validation_precision3, validation_recall3, validation_f1score3 = evaluate_model_supervised(model, validation_loader=learning_routine.test3_loader)
            validation_fpr18, validation_tpr18, validation_auc18, validation_mcc18, validation_precision18, validation_recall18, validation_f1score18 = evaluate_model_supervised(model, validation_loader=learning_routine.test18_loader)
            print(f"Test18AUC: {validation_auc18}")
            print(f"Test3AUC: {validation_auc3}")
            #if validation_loss > 0.77 :
            if validation_auc3 > 0.8 and validation_auc18 > 0.78 and validation_mcc3 > 0.4 and validation_mcc18 >0.4:
                torch.save({
                    'epoch': epoch,
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict()
                }, f'Same6_1024_42_10_{epoch}_{validation_auc3}_{validation_auc18}_{validation_mcc3}_{validation_mcc18}.pth')
        if early_stop:
            pass
            #break
    return learning_routine.best_loss
